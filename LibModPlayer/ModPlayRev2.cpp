...
static void MixAudio(int16_t *Buffer, uint32_t Pos) {
    int i = 0;
	int16_t OutL = 0;
	int16_t OutR = 0;
	float Result = 0;
	if (Playing) {
        while (i < NumOfChannels) {
            if (Ch.Sample != 32 && Ch.Period != 0) {
                ...
                /*
                if (Ch.VolFinal >= Ch.VolFac + VOLRAMPING_SPD)
                    Ch.VolFinal -= VOLRAMPING_SPD ;

                if (Ch.VolFinal <= Ch.VolFac - VOLRAMPING_SPD)
                    Ch.VolFinal += VOLRAMPING_SPD;

                uint8_t VolDelta = Ch.VolFinal - Ch.VolFac;
                if ((VolDelta < VOLRAMPING_SPD) && (VolDelta > -VOLRAMPING_SPD))
                    Ch.VolFinal = Ch.VolFac;
                */
                ...
                if (Smp.RepeatLeng > 2) {
                    ...
                    if ((int32_t)(Ch.Pos>>INT_ACC) - 1 < (int32_t)Smp.RepeatPoint)
                        Ch.Looping = false;
                ...
                if (((Ch.Pos>>INT_ACC) <= Smp.Length) && (Ch.Sample != 32)) {
                    if (Interpolation) {
                        ...
                        if (((Ch.Pos>>INT_ACC) - 1 == Smp.RepeatPoint) && Ch.Looping)
                            PrevData = SampleData[Smp.RepeatPoint + Smp.RepeatLeng];
                    ...
                    if (!Ch.Muted) {
                        if (Ch.Pan != -1) {
                            ...
                            float PanR = 1;
                            if (Ch.Pan < 127)
                                PanR = 1 + (Ch.Pan/127 - 1)*StereoSep;
                            else PanL = 1 - (Ch.Pan - 127)/128*StereoSep;
